...
Production Docker builds receive `API_URL`, `PUBLIC_SITE_URL`, `PUBLIC_SITE_INDEXING`, `PUBLIC_SALES_EMAIL` and `PUBLIC_PRIVACY_EMAIL` as build arguments. The committed production environment deliberately contains placeholders; `npm run build` requires the exact public contract values, rejects invalid or placeholder inputs, and requires indexing to be `true` for the production origin. The sales address drives the Contact page and prerendered `noscript` mailto; the privacy address drives the privacy policy. Neither uses the technical SMTP recipient. The same public origin generates canonical URLs, Open Graph URLs, JSON-LD, `robots.txt` and `sitemap.xml`; the production site uses `index, follow`, while preview and PR builds stay `noindex, follow` and the 404 route remains `noindex, follow`. `API_URL` may remain a technical Cloud Run URL.
