...
This feature preserves the existing public application behavior and adds deployment/operator interfaces. No new business API endpoint is introduced.
...
**Used by**: Local smoke checks, Cloud Run smoke tests, release verification, and rollback validation.
...
- Keeps manual builds safe when `SHORT_SHA` is not present by accepting or deriving a fallback image tag.
...
**Purpose**: Optional Cloud Shell helper for creating or documenting Cloud Build triggers after the repository is already connected.

**Required behavior**

- Accepts project, region, repo, branch, URL, and secret parameters.
- Assumes GitHub-to-Cloud-Build connection already exists.
- Fails clearly with guidance if the repository is not yet connected.
- Does not require secrets or service account keys.

### `scripts/gcp/create-triggers.ps1`

**Purpose**: Optional local PowerShell helper for creating or documenting Cloud Build triggers after the repository is already connected.
...
Must include repository connection steps, exact trigger settings, where substitutions are set, how to verify deployment, how to disable triggers quickly, rollback guidance, and required IAM permissions.

### `infra/gcp/triggers.md`

Must list exact trigger settings for `deploy-prod`, the temporary `002-gcp-deployment` trigger, and the PR validation trigger, plus substitutions and operational notes.
